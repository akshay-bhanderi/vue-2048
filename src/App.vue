<template>
  <div class="max-w-[512px] mx-auto px-8 flex flex-col min-h-screen">
    <div class="flex py-8">
      <div
    ...
    >
      <div ref="gameBoardElement" class="game-grid">
        <div
          class="game-cell"
          v-for="n in state.size * state.size"
          :key="n"
        ></div>
        <div
          ...
          class="game-tile"
          :class="[
            `game-tile-${tile.v > 65536 ? 'super' : tile.v}`,
            { 'game-tile-merged': tile.m },
          ]"
          :style="{ transform: `translate(${tile.x * 100}%,${tile.y * 100}%)` }"
        >
          <transition name="tile" appear>
            <div :innerText="tile.v"></div
          ></transition>
        </div>
      </div>
      <PopupTransitionVue
        ><PopupGameoverVue
          v-if="state.currentGame.isGameover" /></PopupTransitionVue
      ><PopupTransitionVue
        ><PopupSelectGridSizeVue
          v-if="state.showPopupGridSize" /></PopupTransitionVue
      ><PopupTransitionVue>
        <PopupWinVue v-if="state.showPopupWin"
      /></PopupTransitionVue>
    </div>

    <GameControlsVue />
    <div class="text-center mt-auto py-8 text-white font-medium">
      Build with
      <a href="https://vuejs.org/" class="text-emerald-500">Vue.js</a>
    </div>
  </div>
</template>
